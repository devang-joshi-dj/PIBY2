		...
		switch (multiplicationOption) {
			...
			case 'Matrix':
				return rowsSelectedValue && secondMatrixColumnsSelectedValue
					? (setFinalMatrixStructure(
							Array.from({ length: rowsSelectedValue }, () =>
								Array.from({ length: secondMatrixColumnsSelectedValue }, () => '')
							)
					  ),
	...
	const getMatrixMultiplication = () => {
		...
		const PrintMatrixToMatrixMultiplication = () => {
			...
			const PrintAMatrixToBMatrixMultiplication = () => {
				...
				return (
					<>
						{secondMatrix.length && secondMatrix.length === Number(columnsSelectedValue) ? (
							<div className="result">
								<div className="label">
									A
									<ProductSymbol />
		...
		return (
			<>
				<div className="info">What would you do multiplication with?</div>
				<Radio
